...
# --- 1. CONFIGURACIÓN ---
st.set_page_config(page_title="El Faro | Zero Flash", layout="wide", page_icon="⚓")
...
# --- 3. ESTILOS DE INGENIERÍA (EL FIX REAL) ---
# Velocidad dinámica
anim_speed = "0.5s" if st.session_state.search_active else "4s"
...
st.markdown(f"""
    ...
    /* 1. SOLUCIÓN CRÍTICA AL FLASH BLANCO DEL MAPA */
    /* Esto fuerza a que los iframes (mapas) tengan fondo negro antes de cargar */
    iframe {{ background-color: #020617 !important; }}
    .element-container {{ background-color: transparent !important; }}
    
    /* 2. SOLUCIÓN FINAL A LOS TEXTOS DE MÉTRICAS (BLANCO PURO) */
...
# --- 5. SIDEBAR (FARO EN IFRAME AISLADO) ---
# Usamos components.html para aislar la animación del resto de la app
# Esto evita que el navegador repinte el mapa cada vez que la luz gira.
with st.sidebar:
    faro_html = f"""
    <div style="width:100%; height:180px; background:radial-gradient(circle at bottom, #0f172a 0%, transparent 80%); position:relative; overflow:hidden; border-bottom:2px solid #00F0FF; margin-bottom:20px;">
        <div style="position:absolute; bottom:60px; left:50%; margin-left:-300px; width:600px; height:600px; 
             ...
             transform-origin:50% 50%; animation: spin {anim_speed} linear infinite;"></div>
    ...
    st.title("EL FARO")
    st.caption("Sentinel v37.0 | Zero-Flash")
    ...
    if st.session_state.search_active:
        ...
        for i, b in enumerate(bases):
            time.sleep(0.3) # Pequeña pausa para ver la animación rápida
            for _ in range(random.randint(5, 8)):
                titulos = [f"Noticia sobre {obj_in}", f"Polémica en {obj_in}", f"Avance de {obj_in}", f"Crítica a {obj_in}"]
                t = random.choice(titulos)
                s, e, l = normalizar(t, ia)
                data.append({
                    'Fecha': datetime.now().date(), 'Fuente': random.choice(['El Día', 'BioBio', 'Twitter', 'Instagram']),
                    'Titular': t, 'Link': '#', 'Sentimiento': s, 'Alcance': random.randint(1000, 50000),
                    'Interacciones': random.randint(50, 2000), 'Vibra': e, 'Lugar': l, 'Tipo': 'Web'
                })
...
if not df.empty:
    ...
    # KPIs FORZADOS A BLANCO POR CSS
    ...
    pos_perc = int(len(df[df.Sentimiento.str.contains("Positivo")])/len(df)*100)
